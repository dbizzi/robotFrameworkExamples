...
Library             Collections
Library             String
Resource            my_user_and_passwords.robot

*** Variables ***
${GITHUB_HOST}      https://api.github.com
${ISSUES_URI}       /repos/mayribeirofernandes/myudemyrobotframeworkcourse/issues
${ISSUES_COMMENT_URI}       /repos/mayribeirofernandes/myudemyrobotframeworkcourse/issues/12/comments
${ISSUES_LIST_COMMENTS}    /repos/mayribeirofernandes/myudemyrobotframeworkcourse/issues/comments
...
Postando Comentário numa Issue
    # Conectar com autenticação básica na API do GitHub
    Conectar com autenticação por token na API do GitHub
    Postar "Comentário cadastrado via Robot Framework!" como comentário na issue 12

Listar os comentários das issues existentes
    # Conectar com autenticação básica na API do GitHub
    Conectar com autenticação por token na API do GitHub
    CONSULTA OS COMENTÁRIOS das issues existentes
...
Postar "${MENSAGEM}" como comentário na issue 12
    ${BODY}         Format String    ${CURDIR}/data/input/post_issue_comment.json
    ...             mensagem=${MENSAGEM}
    Log             Meu Body ficou:\n${BODY}
    ${RESPONSE}     Post Request    alias=mygithubAuth    uri=${ISSUES_COMMENT_URI}   data=${BODY}
    Confere sucesso na requisição   ${RESPONSE}

CONSULTA OS COMENTÁRIOS das issues existentes
    ${HEADERS}          Create Dictionary    Authorization=Bearer ${MY_GITHUB_TOKEN}    sort=updated    direction=desc
    ${RESPONSE}     Get Request    alias=mygithubAuth    uri=${ISSUES_LIST_COMMENTS}     headers=${HEADERS}
    Confere sucesso na requisição   ${RESPONSE}
